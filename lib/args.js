...
exports.Parser.prototype.command = function (name, handler) {
    ...
    if(handler instanceof exports.Parser) {
        this._commands[name] = function () {
            return handler;
        };
        return;
...
exports.Parser.prototype.act = function (args, options, context) {
    if (!this._action) {
        this.error(options, "Not yet implemented.");
        this.exit(-1);
    }
    options.acted = true;
    this._action.call(this, this.parse(args, {}, false, context), options, context);
};
...
exports.Parser.prototype.parse = function (args, options, noCommand, allowInterleaved, context) {
    var self = this;
    ...
    if(util.has(options, "preActCallback")) {
        if(!options.preActCallback(options, context)) {
            return options;
        }
    }
    ...
    if (util.len(this._commands)) {
        if (args.length) {
            if (util.has(this._commands, args[0])) {
                ...
                command().act(args, options, context);
            } else {
