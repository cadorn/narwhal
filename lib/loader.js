...
exports.reassignFileModule = function(fileModule) {
    file = fileModule;
...
exports.Loader = function (options) {
    ...
    var preprocessors = options.preprocessors || [];
    ...
    loader.find = function (topId) {
        ...
        throw new LoaderError("require error: couldn't find \"" + topId + "\" in ["+searchPaths+"]");
    };
    ...
    loader.fetch = function (topId, path) {
        ...
        if (debug)
            print("loader: fetching '" + topId + "' from: " + path);
        var text = read(path, {
            'charset': 'utf-8'
        });
    ...
    loader.Module = function (text, topId, path) {
        if(preprocessors.length>0) {
            preprocessors.forEach(function(preprocessor) {
                text = preprocessor.process(text, topId, path);
            });
        }
        if (system.evaluate) {
            if (!path)
...
exports.resolvePkg = function(loader, id, baseId, pkg, basePkg) {
    ...
    if(pkg) {
        ...
        if(basePkg && loader.usingCatalog[basePkg]) {
            ...
            if(packages[pkg]) {
                if(loader.usingCatalog[packages[pkg]]) {
                    ...
                    throw new LoaderError("Package '"+packages[pkg]+"' aliased with '"+pkg+"' in '"+basePkg+"' not found");
                }
            }
        }
        ...
        // see if pkg is a uid
        if(loader.uidCatalog[pkg]) {
            var path = loader.usingCatalog[loader.uidCatalog[pkg].id].libPath;
            return [exports.resolve("./" + id, path + "/"), loader.uidCatalog[pkg].id];
        } else {
//            if (system.debug) {
                print("ERROR: " + "Package '" + pkg + "' not aliased in '"+basePkg+"' nor a top-level ID or uid");
                print("  id: "+id);
                print("  baseId: "+baseId);
                print("  pkg: "+pkg);
                print("  basePkg: "+basePkg);
                print("  usingCatalog:");
                for( var key in loader.usingCatalog ) {
                    print("    " + key + " : " + loader.usingCatalog[key].libPath);
                }
                print("  uidCatalog:");
                for( var key in loader.uidCatalog ) {
                    print("    " + key + " : " + loader.uidCatalog[key].id);
                }
//            }
            throw new LoaderError("Package '" + pkg + "' not aliased in '"+basePkg+"' nor a top-level ID or uid");
        }
    } else {
        ...
        if(id.charAt(0) == "." && basePkg) {
            ...
            if(loader.usingCatalog[basePkg]) {
                ...
                throw new LoaderError("basePkg '" + basePkg + "' not known");
            }
...
var LoaderError = exports.LoaderError = function(message) {
