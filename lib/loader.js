...
//print("resolvePkg");    
    ...
    if(loader["pkgCatalog"]) {
        ...
        if(pkg) {
            // check if a system module is requested
/*            
            if(pkg=="system") {
                return [exports.resolve(id, baseId), basePkg];
            }
*/
            
            // locate id in pkg
            if(basePkg && loader.pkgCatalog["localPackages"]["byId"][basePkg]) {
                // see if pkg is an alias                
                var packages = loader.pkgCatalog["localPackages"]["byId"][basePkg].packages;
                if(packages[pkg]) {
                    if(loader.pkgCatalog["localPackages"]["byId"][packages[pkg]]) {
                        var path = loader.pkgCatalog["localPackages"]["byId"][packages[pkg]].libPath;
            ...
            if(loader.pkgCatalog["localPackages"]["byId"][pkg]) {
                var path = loader.pkgCatalog["localPackages"]["byId"][pkg].libPath;
...
//            try {
                ...
                if(id.charAt(0) == "." && basePkg) {
                    ...
                    if(loader.pkgCatalog["localPackages"]["byId"][basePkg]) {
                        path = file.Path(loader.pkgCatalog["localPackages"]["byId"][basePkg].libPath).join(baseId);
                    } else {
...
/*                
                ...
                if(basePkg) {
                    // id relative to basePkg
//                    path = file.Path(loader.pkgCatalog["localPackages"]["byId"][basePkg].libPath);

//print("CHECK AGINST PATH: "+path);
                }
...
print("CHECK AGINST PATH: "+path);
                ...
                loader.find(path.join(id).absolute().valueOf());
//print("CHECK AGINST PATH 22: "+path);
    
                var ret = [exports.resolve(path.join(id).absolute().valueOf()), basePkg];
print("ret: "+ret);
...
//dump(e);
            ...
            // fall back to loading non package scoped system modules
            return [exports.resolve(id, baseId), basePkg];
        }
//print("!!!!proceed");        
*/
/*
        ...
        if(system.fs.Path(callingPkgId).exists() || system.fs.Path(callingPkgId+".js").exists()) {
            ...
            // NEW
            // callingPkgId is a complete package ID
            if(loader.pkgCatalog["localPackages"]["byId"][callingPkgId]) {
                callingPkgInfo = loader.pkgCatalog["localPackages"]["byId"][callingPkgId];
            }
        }
...
if(loader.pkgCatalog["localPackages"]["byId"][pkgId]) {

path = loader.pkgCatalog["localPackages"]["byId"][pkgId].libPath;

                // build module id based on target package lib path
                return [exports.resolve("./" + id, path + "/"), pkgId];
