...
exports.MultiLoader = function (options) {
    ...
    self.preprocessors = options.preprocessors;
    ...
    self.find = function (topId) {
        if(cache) {
            if(cacheDataModule.status=="active") {
                if(cacheDataModule.exports.factories[topId]) {
                    var path = cacheDataModule.exports.factories[topId].path;
    ...
    self.reload = function (topId, loader, path) {
        ...
        if(cache) {
            if(cacheDataModule.status=="collecting") {
                cacheModule(topId, path);
            } else
            if(cacheDataModule.status=="active") {
                if(cacheDataModule.exports.factories[topId] &&
                   cacheDataModule.exports.factories[topId].path==path) {

                    var factory = cacheDataModule.exports.factories[topId].factory;
                    factories[topId] = function(scope) {
                        with(scope) {
                            return factory(require,exports,module,system,print);
                        }
                    }
    ...
    if(cache) {
        cacheFile = require("narwhal/cache").getPath("system", system.sea);
        if(FILE.isFile(cacheFile)) {
            self.loader.evaluate(self.loader.fetch(null, cacheFile), null, cacheFile)(cacheDataModule);
            cacheDataModule.status = "active";
        }
    }

    function cacheModule(topId, path) {

        activateCacheDump();

        if(cacheDataModule.data.modules[topId] &&
           cacheDataModule.data.modules[topId]!=path) {
            throw new Error("Cache conflict for module '" + topId + "' at path: " + path);
        }
        cacheDataModule.data.modules[topId] = path;
    }

    var cacheDumpActivated = false;
    function activateCacheDump() {
        if(cacheDumpActivated) return;
        if (self.isLoaded("unload")) {
            require("unload").when(function() {
                require("loader/cache-writer").write(cacheFile, cacheDataModule.data);
            });
            cacheDumpActivated = true;
        }
    }
