...
parser.option('--location')
    .set()
    .help('The location to overlay.');
...
parser.action(function (options, parentOptions, context) {
    ...
    if(scope=="planet") {
        ...
        if(options.location) {
            ...
            var overlay = OVERLAY.LocationOverlay(options.location, overlayPointer);
            ...
            theme.newMessage({
                "type": "location",
                "scope": scope,
                "location": overlay.getLocationUrl(),
                "overlay": overlay.getOverlayUrl(),
                "message": "Added location overlay to planet"
            }, "{message}").finalize();
        }
    }    
