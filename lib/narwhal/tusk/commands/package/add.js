...
var action = function (options, parentOptions, context) {
    ...
    if(options.catalog) {
        ...
        var uri = URI.parse(options.catalog),
            catalogSelector = options.catalog;

        // see if we have a relative file path to a catalog
        if(uri.protocol=="file" && uri.domain==".") {
            catalogSelector = FILE.Path(FILE.cwd()).join("." + uri.path).absolute().valueOf();
        }

        var catalog = planet.getCatalog(catalogSelector);
        ...
        if(catalog.hasPackage(alias)) {
            if(options.force) {
                catalog.removePackage(alias);
                theme.newMessage({
                    "name": alias,
        ...
        catalog.addPackage(alias, locator);
