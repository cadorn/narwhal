...
parser.action(function (options, parentOptions, context) {
    ...
    if(pkg.exists()) {
     ...
     var seaPkg = sea.getSeaPackage(),
         seaPkgManifest = seaPkg.getManifest(false);
    ...
    catalog.addPackage(name, locator);
    ...
    }, "Added package '{name}' with locator '{locator}' to sea catalog: {catalogPath}").finalize();

    
    // add package as a dependency to the sea package
    var url = catalog.getCatalog().url;
    if(!url) {
    ...
    // now that the package is created we reheat the sea which will also localize it
    tusk.command("tusk sea reheat");
