...
exports.install = function (options, parentOptions, context) {
    ...
    if(uri.protocol=="http" || uri.protocol=="https" || uri.protocol=="file") {
        ...
        if(uri.protocol) {  // a named catalog
            
            // fetch the named catalog
            var catalog = planet.getCatalog(uri.protocol);
            // find the package in the catalog
            locator = catalog.getLocator(packageName);
            // ensure package was found
            if(!locator) {
                theme.newMessage({
                    "packageName": packageName,
                    "catalogUrl": catalog.getUrl(),
                    "message": "Package not found in catalog"
                }, "Package '{packageName}' not found in catalog: {catalogUrl}", "error").finalize();
                return;
            }
            info = false;   // indicate that we do not want to generate a new locator
            // generate a locator for the package in the catalog
            // first we get the original catalog as overlays will be added again later
            while(catalog.hasFallbackCatalog()) {
                catalog = catalog.getFallbackCatalog();
            }
            locator = LOCATOR.Locator({
                "catalog": catalog.getUrl(),
                "name": packageName
            });
        } else {    // the sea catalog
