...
parser.action(function (options, parentOptions, context) {
    ...
    sea.forEachDependency(function(dependency) {
        ...
        info = {
            "name": name,
            "type": (dependency.isSeaPackage())?"local":"dependency",
            "path": pkg.getPath().valueOf(),
            "relativePath": formatRelativePath(seaBasePath, pkg.getPath()),
    ...
    UTIL.keys(packages).forEach(function(name) {
        ...
        if(info.installed) {
            if(info.locator) {
                if(info.type=="local") {
                    line += "\0green({name}\0) - {relativePath} \0yellow(<- {name}\0)";
                } else {
                    line += "\0green({name}\0) - {relativePath} \0magenta(<- {locator}\0)";
                }
            } else {
                line += "\0green({name}\0) - {relativePath}";
            }
        } else
        if(info.dependency) {
            
            line += "\0bold({dependencyType}\0): ";
            line += "\0bold({name}\0) - {relativePath} \0magenta(<- {locator}\0)";
