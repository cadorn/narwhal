...
parser.action(function (options, parentOptions, context) {
    ...
    var seaBasePath = sea.getPath(),
        ...
        localCatalogStruct = localCatalog.getCatalog(false);
    ...
    catalog.forEachPackageDescriptor(function(name, descriptor) {
        ...
        if(// root package
           (name==sea.getSeaPackage().getName()) ||
           // sea package
           (descriptor.path && seaBasePath.join(descriptor.path).exists())) {
            ...
            UTIL.deepUpdate(localCatalogStruct.packages[name], {
                "location": "file://" + seaBasePath.join("").valueOf()
            });
            theme.newMessage({
                "name": name,
            ...
            // fetch the actual package name (not the one used in the catalog)
            if(name!=sea.getSeaPackage().getName()) {
                var pkg = PACKAGE.Package(seaBasePath.join(descriptor.path));
                name = pkg.getName();
            }
    ...
    packages.forEach(function(pkgName) {
        ...
        // repoint location for dependencies of sea packages pointing to sea packages 
        pkg.forEachDependency(function(dependency) {
            
//            if((dependency.getLocator().isNamedOnly() || dependency.isLocalPackage()) &&
            if((dependency.isLocalPackage()) &&
               !dependency.getPackage().getPath().absolute().exists()) {
                ...
                if(dependency.getType()=="package") {
                    ...
                    info.using[dependency.getName()] = {
                        "catalog": "",
                        "location": "file://" + sea.getPackagePath(dependency.getName()).join("").valueOf()
                    };
                } else
