...
PackageMetadata.prototype.forEachOverlay = function(callback, type) {
    if(!type || type=="catalog") {
        if(UTIL.has(this.config, "overlays") && UTIL.has(this.config.overlays, "catalog")) {
            UTIL.every(this.config.overlays.catalog, function(pair) {
                callback(OVERLAY.CatalogOverlay(pair[0], pair[1]));
            });
        }
    }
}
