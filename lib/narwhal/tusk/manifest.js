...
var Manifest = exports.Manifest = function (path) {
    ...
    this.path = path;
    this.manifest = (path && path.exists())?json.decode(path.read({charset:"utf-8"})):{schema: "1.1"};
}
...
Manifest.prototype.getTmpDirectory = function() {
    // TODO: check manifest for alternative build path
    return this.path.dirname().join(".tmp");
}
