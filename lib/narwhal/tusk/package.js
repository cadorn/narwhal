...
var Package = exports.Package = function (path, locator) {
    ...
    if(PACKAGES.usingCatalog[path]) {
        this.id = path;
        this.path = PACKAGES.usingCatalog[path].directory;
    } else {
        this.path = fs.Path(path);
    }
    this.locator = locator;
    if(!this.id && locator) {
        this.id = locator.getOriginalId();
    }
...
// needed
// TODO: This needs simplification now that we have ground rules set
Package.prototype.setSea = function(sea) {
    ...
    if(!this.id) {
        ...
        if(!this.exists()) {
            this.id = false;
/*            
            this.id = this.getPath().valueOf();

            var usingBasePath = sea.getDependenciesPath().valueOf();
            
            // check if id (path) falls within 'using/' dir
            if(this.id.substr(0, usingBasePath.length)==usingBasePath) {
                this.id = this.id.substr(usingBasePath.length+1);
                var path;
                if((path = sea.getPackagePath(this.id)).exists()) {
                    this.id = path.absolute().valueOf().substr(1);
                }
            }
*/        
            return this;
        ...
        if(this.getPath().valueOf().substr(basePath.length+1)==this.getName()) {
            this.id = this.getName();
        } else {
        
            basePath = sea.getDependenciesPath().valueOf();
...
Package.prototype.getManifest = function(includeLocal) {
    ...
    // overlay local package metadata
    if(!this.ignoreLocalManifest && includeLocal && this.localManifest.exists()) {
        ...
        mergedManifest.manifest = util.deepCopy(this.manifest.manifest);
...
//                    path.rmtree();
    ...
    // if no package.json file is found in final path we generate one
    if(!path.join("package.json").exists()) {
        path.join("package.json").write(json.encode({
            "name": this.getId()
...
Package.prototype.installDependencies = function(type) {
    ...
    type = type || "package";
    if(!util.isArrayLike(type)) type = [type];
    ...
    types.forEach(function(type) {
        ...
        if(dependencies) {
            util.every(dependencies, function(pair) {
                ...
                if(visited.indexOf(type+":"+pkg.getPath())>=0) {
                    return;
                }
                visited.push(type+":"+pkg.getPath());
