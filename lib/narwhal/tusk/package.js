...
// needed
Package.prototype.forEachDependency = function(callback, type, deep, visited) {
    ...
    if(type=="package") {
        ...
        if(localManifest.exists()) {
            ...
            if(localDependencies) {
                util.update(dependencies, localDependencies);
            }
        }
    } else
    if(type=="build") {
    ...
    util.every(dependencies, function(pair) {
        var dep = DEPENDENCY.Dependency(self.sea, self, pair[0], pair[1]),
            pkg = dep.getPackage();

        if(visited.indexOf(pkg.getId())>=0) {
            return;
        }
        visited.push(pkg.getId());
        
        callback(dep);

        if(deep) {
            visited = pkg.forEachDependency(callback, type, deep, visited);
        }
    });
    return visited;
}
...
Package.prototype.getDependency = function(name) {
    ...
    return DEPENDENCY.Dependency(self.sea, self, name, dependencies[name]);
}
