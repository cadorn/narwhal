...
exports.Planet = function (planetPath) {
    ...
    Planet.getCatalog = function(selector, includeOverlay) {
...
//            return CATALOG.Catalog(TUSK.getNarwhalHomePath().join("catalog.json"));
        ...
        if((path = FILE.Path(selector)).exists()) {
            ...
            if(uri.protocol=="file" && FILE.path(uri.url.substr(7)).exists()) {
                catalog = CATALOG.Catalog(uri.url.substr(7));
            } else {
...
throw "implement Planet.getPackage() for catalogs";            
