...
var Sea = exports.Sea = function (path) {
    ...
    this.usingPackages = {};
    this.uidCatalog = {};
    this.discoverPackages(path);
}

Sea.prototype.discoverPackages = function(path) {
    var self= this;
    
    try {
    
        PACKAGES.readUsing({}, this.usingPackages, this.uidCatalog, path.join("using"));
        
        if(!path.join("packages").exists()) {
            return;
        }
        path.join("packages").listPaths().forEach(function(item) {
            if(item.isDirectory()) {
                var pkg = PACKAGE.Package(item);
                self.packages[pkg.getName()] = pkg.getPath().valueOf();
                self.discoverPackages(item);
            }
        });
    } catch(e) {
        print(e);
        print("ERROR IN PACKAGE: "+this.path);
        throw e;
    }
}
...
*/
        ...
        packageInfo = json.decode(packagePath.read({charset:'utf-8'}));
    }
    
    // create catalog
    if(!path.join("catalog.json").exists()) {
        CATALOG.Catalog(path.join("catalog.json")).create();
    }
    ...
    if(!sea.exists()) {
        ...
        var customizations = [
            // OSX Terminal
            'export PS1="\\e[00;35mTUSK[\\e[00;33m' + packageInfo.name + '\\e[00;35m]:\\e[m "',
            // OSX iTerm (http://xanana.ucsc.edu/xtal/iterm_tab_customization.html)
            'echo -ne "\\e]1;TUSK[' + packageInfo.name + ']\\a"',
            'echo -ne "\\e]2;TUSK[' + packageInfo.name + '] @ ' + path.valueOf() + '\\a"',
            'cd $PACKAGE_HOME'
        ];
...
Sea.prototype.getPackage = function(name) {
    ...
    var path;
    // new way to get packages
    // THIS SHOULD CAPTURE ALL REQUESTS
    if(this.packages[name]) {
        path = this.packages[name];
    } else
    if(this.usingPackages[name]) {
        path = this.usingPackages[name].directory;
    } else {
        // LEGACY I THINK
...
// needed
Sea.prototype.getTmpDirectory = function() {
    var path = this.getPackageManifest().getTmpDirectory();
    path.mkdirs();
    return path;
}
