...
/*        
        if(args[0]!="tusk") {
            ...
            var command = Tusk.getSea().getBinPath().join("").valueOf() + args.join(" ");

            var process = OS.popen(command);
            var result = process.communicate();
            if(result.status!==0) {
                throw new exports.TuskError("Program exited unexpectedly");
            }
            
            // TODO: Return proper themed response
...
exports.getActiveSea = function() {
    var path = SYSTEM.env["SEA"];
    if(!path) {
        path = FILE.path(SYSTEM.prefixes[0]);
    }
    return SEA.Sea(path);
}
