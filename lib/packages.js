...
exports.normalizePkgLocator = function(locator, resolveSeaIds) {
    ...
    if(util.has(locator, "catalog")) {
        ...
        // if we have a relative file URL that points to the sea catalog
        // we assume the package is a top-level ID
        // TODO: Do a file exists on the relative path
        if(uri.protocol=="file" && uri.domain.substr(0,1)==".") {
            return locator["package"];
        }
    } else
