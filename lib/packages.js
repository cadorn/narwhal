        ...
        while (queue.length) {
            ...
            if(name=="dependencies") {
                ...
                try {
                    ...
                    // overlay local package file if the package is not linked into the sea
                    var localOverlay = packageDirectory.join('package.local.json');
                    if (localOverlay.isFile() && packageDirectory.canonical().valueOf()==packageDirectory.absolute().valueOf()) {
                    ...
                    if(util.has(packageDatum, "using")) {
                        util.every(packageDatum.using, function(pair) {
                            pkgCatalogDatum["packages"][pair[0]] = exports.normalizePackageDescriptor(pkgCatalog, pair[1]);
                        });
                    }
                    ...
                    pkgCatalog["localPackages"]["byPath"][packageDatum.directory.valueOf()] = name;
...
exports.normalizePackageDescriptor = function(pkgCatalog, descriptor) {
    if(!descriptor) return descriptor;
    var uri,
        path = "";
    if(util.has(descriptor, "location") && descriptor.location) {
        // if a name is specified with the location we use the name as the
        // top-level ID instead of a dependency ID
        if(util.has(descriptor, "name") && descriptor.name) {
...
dump(descriptor);
    ...
    var id = fs.Path("dependencies").join(uri.domain + uri.path).dirname();
