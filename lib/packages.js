...
exports.main = function main(options) {
    ...
    exports.load(system.packages.concat(system.prefixes), options);
...
exports.load = function (prefixes, options) {
    ...
    var cache = options.cache || false,
        cacheFile = (cache)?fs.Path(CACHE.getPath("packages", [
            prefixes,
            options
        ])):null,
        cacheData;
    ...
    var catalog = {},
        ...
        uidCatalog = require.loader.uidCatalog || {},
        root,
        order;
    
    if(!cache || !(cacheData = loadCache(cacheFile))) {

        // depth first search of the packages tree and roots
        root = exports.read(prefixes, catalog, usingCatalog, uidCatalog, options);
        ...
        // components
        order = exports.sortedPackages(catalog);

        if(cache) {
            CACHE.write(cacheFile, encodeForCache({
                root: root,
                catalog: catalog,
        ...
        root = cacheData.root;
        catalog = cacheData.catalog;
        usingCatalog = cacheData.usingCatalog;
        uidCatalog = cacheData.uidCatalog;
        order = cacheData.order;
    }
        
    // analysis
    var analysis = {};
    exports.analyze(analysis, order, usingCatalog);
...
exports.registerUsingPackage = function(locator, path) {
    if(!(path instanceof fs.Path)) {
        path = fs.Path(path);
    }
    if(!path.join("package.json").exists()) {
        throw new Error("No package at: " + path);
    }
    var descriptor = json.parse(path.join("package.json").read().toString()),
        id,
        localOverlay = path.join('package.local.json');
    if (localOverlay.isFile()) {
        util.deepUpdate(descriptor, json.parse(localOverlay.read().toString()));
    }
    id = exports.normalizePackageDescriptor(locator);

    exports.updateUsingCatalog({}, exports.usingCatalog, exports.uidCatalog, path, id, descriptor);

    // update catalogs in loader
    require.loader.usingCatalog = exports.usingCatalog;
    require.loader.uidCatalog = exports.uidCatalog;
}


function encodeForCache(data) {
    var jsonifiableData = {
        "root": {},
        "catalog": {},
        "usingCatalog": {},
    ...
    var obj = util.copy(data.root);
        obj.directory = obj.directory.valueOf();
        obj.contributors = obj.contributors.map(function(author) {
            return author.getRaw();
        });
    jsonifiableData.root = obj;
    
    util.every(data.catalog, function(module) {
        var obj = util.copy(module[1]);
        if(obj.author) obj.author = obj.author.getRaw();
        obj.directory = obj.directory.valueOf();
        obj.contributors = obj.contributors.map(function(author) {
            return author.getRaw();
        });
        jsonifiableData.catalog[module[0]] = obj;
    });
    
    util.every(data.usingCatalog, function(module) {
        var obj = util.copy(module[1]);
        obj.libPath = obj.libPath.valueOf();
        obj.directory = obj.directory.valueOf();
        jsonifiableData.usingCatalog[module[0]] = obj;
    });

    jsonifiableData.uidCatalog = data.uidCatalog;
    
    jsonifiableData.order = util.map(data.order, function(module) {
        return module.name;
    });
    
    return json.encode(jsonifiableData, null, "  ");
}

function loadCache(file, targets) {
    if(!file.exists()) {
        return false;
    }
    
    var rawData = json.decode(file.read()),
        data = {
            "root": {},
            "catalog": {},
            "usingCatalog": {},
    ...
    var obj = rawData.root;
    obj.directory = fs.Path(obj.directory);
    obj.contributors = obj.contributors.map(function(author) {
        return new exports.Author(author);
    });
    data.root = obj;

    util.every(rawData.catalog, function(module) {
        var obj = module[1];
        if(obj.author) obj.author = new exports.Author(obj.author);
        obj.directory = fs.Path(obj.directory);
        obj.contributors = obj.contributors.map(function(author) {
            return new exports.Author(author);
        });
        data.catalog[module[0]] = obj;
    });

    util.every(rawData.usingCatalog, function(module) {
        var obj = module[1];
        obj.libPath = fs.Path(obj.libPath);
        obj.directory = fs.Path(obj.directory);
        data.usingCatalog[module[0]] = obj;
    });

    data.uidCatalog = rawData.uidCatalog;

    data.order = util.map(rawData.order, function(module) {
        return data.catalog[module];
    });
...
*/
exports.read = function read(prefixes, catalog, usingCatalog, uidCatalog, options) {
    ...
    // using packages may declare system dependencies which we need to collect and subsequently load
    var systemDependencies = [];

    prefixes.forEach(function(prefix) {
        exports.readUsing(options, systemDependencies, usingCatalog, uidCatalog, fs.Path(prefix).join("using"));
    });

    if(system.sea && !util.has(system.sea, prefixes)) {
        exports.readUsing(options, systemDependencies, usingCatalog, uidCatalog, fs.Path(system.sea).join("using"));
    }
    ...
    while (prefixes.length) {
        ...
        systemDependencies.forEach(function(dependencies) {
            scanDependencies(queue, dependencies);            
        })
        ...
        while (queue.length) {
            ...
            try {
                ...
                /*
                 * Now searching in "system.sea" (see above)
                exports.readUsing(options, usingCatalog, uidCatalog, packageDirectory.join("using"));
                */
                ...
                if(util.has(packageDatum, "uid")) {
                    if(!uidCatalog[packageDatum.uid]) uidCatalog[packageDatum.uid] = {};
                    uidCatalog[packageDatum.uid].id = name;
                }

                // add this system package to the usingCatalog
                exports.updateUsingCatalog(options, usingCatalog, uidCatalog, packageDirectory, name, packageDatum);
                ...
                if(packageDatum.dependencies) {
                    scanDependencies(queue, packageDatum.dependencies);
                }
    ...
    function scanDependencies(queue, dependencies) {
        var names = [];
        if(!util.isArrayLike(dependencies)) {
            util.every(dependencies, function(dependency) {
                // do we have a locator
                if(util.has(dependency[1], "catalog") || util.has(dependency[1], "location")) {
                    names.push(exports.normalizePackageDescriptor(dependency[1]));
                } else {
                    // we are requesting a specific version - use the name and ignore version
        ...
        names.forEach(function(dependency) {
...
var scan = function scan(catalog, name) {
    ...
    try {
        if (packageDatum.dependencies) {
            var dependencies = packageDatum.dependencies;

            // locator-based dependencies
            if(!util.isArrayLike(dependencies)) {
                var names = [];
                util.every(dependencies, function(dependency) {
                    // do we have a locator
...
*/
exports.analyze = function analyze(analysis, catalog, usingCatalog) {
    analysis.libPaths = [];
...
// HACK @cadorn - This can be removed once FILE <-> URI is working and narwhal-xulrunner engine is cleaned up                
                if(engineLibs[i].substr(0,10)=="resource:/" || engineLibs[i].substr(0,8)=="chrome:/") {
                    engineLibs[i] = engineLibs[i];
                } else {
// HACK
    ...
    util.every(usingCatalog, function (info) {
        if(info[1].descriptor.preload) {
            if (typeof info[1].descriptor.preload == "string")
                info[1].descriptor.preload = [info[1].descriptor.preload];
            // preload modules referenced with "./" are local to the package
            for( var i=0 ; i<info[1].descriptor.preload.length ; i++ ) {
                if(!util.isArrayLike(info[1].descriptor.preload[i]) && info[1].descriptor.preload[i].substring(0,2)=="./") {
                    info[1].descriptor.preload[i] = [
                        info[1].descriptor.preload[i].substring(2),
                        info[0]
                    ]
                }
            }
            analysis.preloadModules.unshift.apply(analysis.preloadModules, info[1].descriptor.preload);
        }
    });
};
...
exports.normalizePackageDescriptor = function(descriptor) {
    ...
    if(util.has(descriptor, "catalog") && descriptor.catalog) {
        ...
        if(util.has(descriptor, "revision")) revision = descriptor.revision;
    } else {
...
exports.readUsing = function(options, systemDependencies, usingCatalog, uidCatalog, basePath, subPath) {
    ...
    if(path.join("package.json").exists()) {
        ...
        if(packageDatum.dependencies) {
            systemDependencies.push(packageDatum.dependencies);
        }

        var id = subPath.valueOf().replace(/\\/g, "/");	// windows compatibility
        
        exports.updateUsingCatalog(options, usingCatalog, uidCatalog, path, id, packageDatum);
        ...
        path.listPaths().forEach(function(dir) {
            exports.readUsing(options, systemDependencies, usingCatalog, uidCatalog, basePath, subPath.join(dir.basename()));
        });
    }
}

exports.updateUsingCatalog = function(options, usingCatalog, uidCatalog, path, id, packageDatum) {
    if(util.has(packageDatum, "uid")) {
        if(!uidCatalog[packageDatum.uid]) uidCatalog[packageDatum.uid] = {};
        uidCatalog[packageDatum.uid].id = id;
    }
    if(!util.has(usingCatalog, id)) {
        usingCatalog[id] = {
            "uid": packageDatum.uid || "",
            "libPath": path.join("lib"),
            "directory": path,
            "packages": {},
            "descriptor": packageDatum
        };
    }
...
exports.Author = function (author) {
    ...
    this.getRaw = function() {
        if(author instanceof exports.Author) {
            return author.getRaw();
        } else {
            return author;
        }
