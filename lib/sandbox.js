...
//print("id["+id+"] baseId["+baseId+"] pkg["+pkg+"] basePkg["+basePkg+"]");            
        ...
        if(loader["resolvePkg"]) {
            idContext = loader.resolvePkg(id, baseId, pkg, basePkg);
            id = idContext[0];
            pkg = idContext[1];
        } else {
...
//print("  USING: id["+id+"] pkg["+pkg+"]");
 ...
 //           var pkgId = loader.pkgForPath(factory.path);

            var require = Require(id, factory.path, pkg);
            var module = {
                'id': id,
                'path': factory.path
            };
            if(pkg) {
                module["package"] = pkg;
                module["packages"] = ((pkg && loader.localPackages.byId[pkg])?loader.localPackages.byId[pkg]["packages"]:{});
            }
